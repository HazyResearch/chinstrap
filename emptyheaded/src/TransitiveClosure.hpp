...
namespace tc {
  ...
  TrieBlock<T,R>* unweighted_single_source(
    ...
    const size_t visited_num_bytes = PADDING*range_bitset::get_number_of_bytes(num_distinct,num_distinct);
    ...
    ops::atomic_union(&visited->set,PADDING*start);
    ...
    debug::stop_clock("Iteration " + iteration,it_timer);
