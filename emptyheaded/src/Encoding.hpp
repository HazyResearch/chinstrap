...
public:
  ...
  Encoding(std::vector<Column<T>*> *attr_in){
    encoded = new std::vector<Column<uint32_t>*>();
    key_to_value = new std::unordered_map<T,uint32_t>();
    value_to_key = new std::vector<T>();

    uint32_t index = 0;
    const size_t num_attributes = attr_in->size();
    for(size_t i = 0; i < num_attributes; i++){
      const Column<T> *input = attr_in->at(i);
      Column<uint32_t> *output = new Column<uint32_t>();
